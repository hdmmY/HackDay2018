...
public class AttractBulletMoveCtrl : BaseBulletMoveCtrl
{
    ...
    private void Update ()
    {
        ...
        Vector2 newPos = Vector2.SmoothDamp (current, target, ref velocity,
            SmoothFactor * distance, 10f, AttractFactor * Time.deltaTime);

        _bulletMove.Speed = (newPos - current).magnitude / Time.deltaTime;
        _bulletMove.AngleSpeed = 0f;

        Quaternion rot = Quaternion.Euler (0, 0,
            Utility.GetAngleFromTwoPosition (current, newPos) - 90f);
